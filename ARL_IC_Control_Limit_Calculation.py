...
def simulate_Q(alpha):
    ...
    print(f"Alpha: {alpha:.6f}, ARL_Q: {ARL_Q:.4f}") 
...
def simulate_SSMEWMA(h):
    ...
    for run in range(runs):
        ...
        cluster_data = apply_fixed_clustering(full_data, 2, data_real)
    ...
    print(f"Alpha: {h:.6f}, ARL_Q: {ARL_SSEWMA:.4f}") 
...
def simulate_PSSMEWMA(h):
    ...
    for run in range(runs):
        ...
        for current_run_length in range(2,run_length,1):
            ...
            clustered_data_DBSCAN, cluster_data_noise, noise_cluster_number, total_cluster, biggest_cluster, cluster_matrix, T_list = find_groups(current_data.T, full_data,T_list, cluster_matrix)
            result_SSEWMA, result_Q_chart, count_combi = get_combined_results(count_combi, count_combi_Q, current_data, filepath, filename_Combi_Q, ARL_list_Combi, clustered_data_DBSCAN, cluster_data_noise, filename_Combi_SSEWMA, h)  
...
def simulate_HC(h):
    ...
    print(f"Alpha: {h:.6f}, ARL_CP: {ARL_CP:.4f}") 
